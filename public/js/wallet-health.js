...
function WalletHealthApp() {
  ...
  async function loadWalletHealth() {
    setLoading(true);
    try {
      const res = await fetch("/api/wallet-health", {
        credentials: "include"
      });
  ...
  const filteredRows = useMemo(() => {
    ...
    const excludedOwnerTerms = filters.excludeOwner.split(",").map(term => term.trim().toLowerCase()).filter(Boolean);
    return rows.filter(row => {
      const ownerName = String(row.ownerName || "").toLowerCase();
      const searchable = [row.walletId, row.ownerName, row.deviceName, row.deviceId, row.agentGroup].join(" ").toLowerCase();
      ...
      if (filters.status) {
        ...
        } else if (normalize(row.depositStatus) !== filterStatus && normalize(row.withdrawalStatus) !== filterStatus) {
          return false;
        }
      }
  ...
  const selectedFilteredIds = useMemo(() => filteredRows.filter(row => selectedIds.has(row.id)).map(row => row.id), [filteredRows, selectedIds]);
  useEffect(() => {
    setCurrentPage(1);
  }, [filters]);
  const stats = useMemo(() => {
    ...
    const unavailable = filteredRows.filter(row => normalize(row.depositStatus) !== "AVAILABLE" || normalize(row.withdrawalStatus) !== "AVAILABLE").length;
  ...
  async function sendSelectedNotice(messageType = "health_report") {
    ...
    try {
      ...
      const missing = data.missingGroups?.length ? ` Missing Chat ID: ${data.missingGroups.join(", ")}` : "";
  ...
  async function sendOldAppNotice() {
    const selectedRows = selectedFilteredIds.length ? filteredRows.filter(row => selectedIds.has(row.id)) : filteredRows;
    const rowIds = selectedRows.filter(row => isOldAppVersion(row.appVersion, targetVersion)).map(row => row.id);
  ...
  const visibleGroups = useMemo(() => {
    ...
    filteredRows.forEach(row => {
      ...
      current.total += 1;
      if (normalize(row.appCondition) !== "HEALTHY" || normalize(row.depositStatus) !== "AVAILABLE" || normalize(row.withdrawalStatus) !== "AVAILABLE" || isOldAppVersion(row.appVersion, targetVersion)) {
        current.bad += 1;
      }
      map.set(key, current);
    });
    return Array.from(map.entries()).map(([agentGroup, value]) => ({
      agentGroup,
      ...value
    })).sort((a, b) => b.bad - a.bad || a.agentGroup.localeCompare(b.agentGroup)).slice(0, 8);
  }, [filteredRows, targetVersion]);
  const pageSize = 50;
  ...
  return React.createElement("div", {
  ...
  }, React.createElement("style", null, walletHealthCss), React.createElement("div", {
    className: "wh-header"
  }, React.createElement("div", null, React.createElement("span", {
    className: "wh-kicker"
  }, "Wallet App Health"), React.createElement("h2", null, "Active Device Monitor"), React.createElement("p", null, "Upload the health CSV, review device condition, then send a clean text report to Telegram by Agent Group.")), React.createElement("div", {
    className: "wh-upload"
    ...
    onClick: uploadFile,
    disabled: uploading
  }, uploading ? "Uploading..." : "Upload Health CSV"), React.createElement("button", {
    className: "danger",
    onClick: resetHealthData,
    disabled: loading
  }, "Reset"), React.createElement("button", {
    className: "secondary",
    onClick: loadWalletHealth,
    disabled: loading
  }, loading ? "Loading..." : "Reload"))), React.createElement("div", {
    className: "wh-stats"
    ...
    label: "Unavailable DP/WD",
    value: stats.unavailable,
    tone: "blue"
  }), React.createElement(StatCard, {
    label: `Old App${targetVersion ? ` < ${targetVersion}` : ""}`,
    value: stats.oldApp,
    tone: "purple"
  })), React.createElement("div", {
    className: "wh-panel"
  }, React.createElement("div", {
    className: "wh-filters"
  }, React.createElement("input", {
    ...
    onChange: event => setFilters({
      ...filters,
      excludeOwner: event.target.value
    }),
    placeholder: "Exclude owner contains, e.g. test"
  }), React.createElement(Select, {
    label: "Wallet Type",
    value: filters.walletType,
    options: options.walletTypes,
    onChange: walletType => setFilters({
      ...filters,
      walletType
    })
  }), React.createElement(Select, {
    ...
    onChange: teamLeader => setFilters({
      ...filters,
      teamLeader
    })
  }), React.createElement(Select, {
    label: "Agent Group",
    value: filters.agentGroup,
    options: options.agentGroups,
    onChange: agentGroup => setFilters({
      ...filters,
      agentGroup
    })
  }), React.createElement(Select, {
    ...
    onChange: accountType => setFilters({
      ...filters,
      accountType
    })
  }), React.createElement(Select, {
    ...
    onChange: appVersion => setFilters({
      ...filters,
      appVersion
    })
  }), React.createElement(Select, {
    ...
    onChange: condition => setFilters({
      ...filters,
      condition
    })
  }), React.createElement("div", {
    className: "wh-status-filter"
  }, React.createElement(Select, {
    label: "Status",
    ...
    onChange: status => setFilters({
      ...filters,
      status,
      availableSide: normalize(status) === "AVAILABLE" ? filters.availableSide || "deposit" : filters.availableSide
    })
  }), normalize(filters.status) === "AVAILABLE" && React.createElement("div", {
    className: "wh-available-toggle"
  }, React.createElement("span", null, "Available"), React.createElement("button", {
    type: "button",
    ...
    onClick: () => setFilters({
      ...filters,
      availableSide: "withdrawal"
    })
  }, "Withdrawal"), React.createElement("button", {
    type: "button",
  ...
  }, "Both"))), React.createElement("button", {
    className: "ghost",
    onClick: () => {
      setFilters({
        search: "",
    ...
    className: "wh-version-tools"
  }, React.createElement("label", null, "Latest App Ver", React.createElement("input", {
    value: targetVersion,
    onChange: event => setTargetVersion(event.target.value),
    placeholder: "Example: 2.0.9"
  })), React.createElement("button", {
    className: oldAppOnly ? "active" : "",
    onClick: () => setOldAppOnly(value => !value)
  }, oldAppOnly ? "Showing Old App Only" : "Filter Old App"), React.createElement("span", null, stats.oldApp, " wallet", stats.oldApp === 1 ? "" : "s", " below latest version")), React.createElement("div", {
    className: "wh-sendbar"
  }, React.createElement("div", null, React.createElement("b", null, "Telegram mapping"), React.createElement("span", null, "Checked rows send only selected wallets. Old App Notice uses checked rows, or the current filtered old-app rows if nothing is checked.")), React.createElement("button", {
    className: "primary",
    onClick: () => sendSelectedNotice("health_report"),
    disabled: selectedFilteredIds.length === 0 || Boolean(sending)
  }, sending === "relog notice" ? "Sending relog..." : `Send Relog Notice (${selectedFilteredIds.length})`), React.createElement("button", {
    className: "info",
    onClick: () => sendSelectedNotice("permission"),
    disabled: selectedFilteredIds.length === 0 || Boolean(sending)
  }, sending === "permission notice" ? "Sending permission..." : `Send Permission Notice (${selectedFilteredIds.length})`), React.createElement("button", {
    className: "warning",
    onClick: sendOldAppNotice,
    disabled: !targetVersion.trim() || stats.oldApp === 0 || Boolean(sending)
  }, sending === "old app notice" ? "Sending notice..." : "Send Old App Notice"), React.createElement("button", {
    className: "secondary",
    onClick: () => sendGroup(),
    disabled: !filters.agentGroup || Boolean(sending)
  }, sending ? `Sending ${sending}...` : "Send Filtered Group"))), React.createElement("div", {
    className: "wh-groups"
  }, visibleGroups.map(group => React.createElement("button", {
    key: group.agentGroup,
    className: "wh-group",
    onClick: () => sendGroup(group.agentGroup),
    disabled: Boolean(sending)
  }, React.createElement("span", null, group.agentGroup), React.createElement("small", null, group.total, " wallets / ", group.bad, " issues")))), React.createElement("div", {
    className: "wh-pagination"
  }, React.createElement("span", null, "Showing ", filteredRows.length ? pageStart + 1 : 0, "-", Math.min(pageStart + pageSize, filteredRows.length), " of ", filteredRows.length), React.createElement("div", null, React.createElement("button", {
    className: "secondary",
    ...
    disabled: safePage === 1
  }, "Prev"), React.createElement("b", null, "Page ", safePage, " / ", totalPages), React.createElement("button", {
    className: "secondary",
  ...
  })), React.createElement("th", null, "Wallet"), React.createElement("th", null, "Account"), React.createElement("th", null, "Status"), React.createElement("th", null, "Owner"), React.createElement("th", null, "Team / Group"), React.createElement("th", null, "Condition"), React.createElement("th", null, "Deposit"), React.createElement("th", null, "Withdrawal"), React.createElement("th", null, "API Balance"), React.createElement("th", null, "Device"), React.createElement("th", null, "App Ver"), React.createElement("th", null, "SMS"), React.createElement("th", null, "Listener"), React.createElement("th", null, "App Noti"), React.createElement("th", null, "Full Screen"), React.createElement("th", null, "Battery"), React.createElement("th", null, "Last Active"), React.createElement("th", null, "API Fails"))), React.createElement("tbody", null, visibleRows.map(row => React.createElement("tr", {
    key: row.id,
  ...
  })), React.createElement("td", null, React.createElement("b", null, row.walletId || "-"), React.createElement("small", null, row.walletType || "-")), React.createElement("td", null, React.createElement(Pill, {
    value: row.accountType
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.walletActive
  })), React.createElement("td", null, React.createElement("b", null, row.ownerName || "-"), React.createElement("small", null, "PA #", row.personalAccountId || "-")), React.createElement("td", null, React.createElement("b", null, row.teamLeader || "-"), React.createElement("small", null, row.agentGroup || "-")), React.createElement("td", null, React.createElement(Condition, {
    row: row
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.depositStatus
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.withdrawalStatus
  })), React.createElement("td", {
    className: "wh-money"
  }, formatWalletAmount(row.apiBalance)), React.createElement("td", null, React.createElement("b", null, row.deviceName || "-"), React.createElement("small", null, row.deviceId || "-")), React.createElement("td", null, row.appVersion || "-"), React.createElement("td", null, React.createElement(Pill, {
    value: row.smsPermission
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.notificationListener
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.appNotifications
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.fullScreenAlert
  })), React.createElement("td", null, React.createElement(Pill, {
    value: row.batteryOptimizationDisabled
  })), React.createElement("td", null, row.lastActive || "-"), React.createElement("td", null, React.createElement(Pill, {
    value: String(row.apiFailures ?? 0)
  }))))))), React.createElement("div", {
    className: "wh-footnote"
  }, "50 rows per page. Header checkbox selects this page only."));
}
function StatCard({
  label,
  value,
...
function Select({
  ...
  return React.createElement("select", {
    ...
    value: ""
  }, label), options.map(option => React.createElement("option", {
    key: option,
    value: option
  }, option)));
}
...
function Condition({
  ...
  return React.createElement("div", {
    className: "wh-condition"
  }, React.createElement(Pill, {
    value: row.appCondition
  }), React.createElement("small", {
    className: normalize(row.appCondition) === "SYNC_DELAYED" ? "warning" : ""
...
function uniqueVersions(values) {
  return Array.from(new Set(values.filter(Boolean))).sort((a, b) => compareVersions(b, a) || String(b).localeCompare(String(a)));
}
...
if (walletHealthRoot) {
  ReactDOM.createRoot(walletHealthRoot).render(React.createElement(WalletHealthApp, null));
}
